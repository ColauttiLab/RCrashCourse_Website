...
We will explore the *ggplot2* package for graphing.

It was created by the same group that also made:

*RStudio*, *RMarkdown*, *Shiny*, *ggvis*

and many other cool packages. 
...
***qplot()*** is a quick and simple yet versatile plotting function.

It is much more intuitive than R's default graphics commands.
...
qplot(x=Total,data=MyData,group=Nutrients,fill=Nutrients,position="dodge")
```
...
#### Simplified/clean theme more suitable for presentation or publication:
```{r}
...
#### What it looks like:
```{r}
qplot(x=Silene,data=MyData) + theme_pubworth() # A clean format with bigger axis labels for publication
```
