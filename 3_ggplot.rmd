...
CLab <- function (base_size = 12, base_family = "") {
...
```{r, echo=F}
qplot(X,Y) + scale_y_continuous(limits=c(0,max(100))) + CLab()
```
...
qplot(Lat,Long,colour=Temp, data=D) + scale_color_gradient(high="blue", low="red") + CLab()
```
...
qplot(Lat,Long,colour=Precip, data=D) + scale_color_gradient(high="#56B4E9", low="#56B499") + CLab()
```

## 6. Keep ielevant information
...
X<-rnorm(100)
Y<-X+rnorm(100)
qplot(c(X,Y),fill=c(rep("X",100),rep("Y",100)),posit="dodge") + CLab()
qplot(X,Y) + CLab()
```
...
To get to know ggplot better, let's do a step-by-step example of a figure built for a [paper by Colautti & Lau](https://doi.org/10.1111/mec.13162) in the journal Molecular Ecology (2015)
